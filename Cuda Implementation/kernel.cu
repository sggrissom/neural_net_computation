...
void cpu_bpgt(float* in,
	...
	for(i=1;i<numl;i++){				// For each layer
		for(j=0;j<lsize[i];j++){		// For each neuron in current layer
			sum=0.0;
			for(k=0;k<lsize[i-1];k++){		// For input from each neuron in preceeding layer
				sum+= out[i-1][k]*weight[i][j][k];	// Apply weight to inputs and add to sum
			}
			sum+=weight[i][j][lsize[i-1]];		// Apply bias
			out[i][j]= (float) (1/(1+exp(-sum))); 		// Apply sigmoid function
		}
	}
		...
		)
{
	...
	for(i=1;i<numl;i++){				// For each layer
		for(int j=0;j<lsize[i];j++){		// For each neuron in current layer
			...
			sum+=weight[i][j][lsize[i-1]];		// Apply bias
			out[i][j]= (1/(1+exp(-sum)));// Apply sigmoid function
		}
	}
}

float cpu_mse(float *tgt, float **out, int* lsize, int numl)
{
	float mse=0;
